...
// Ensure currentPage and totalPages are numbers
  ...
  return (
    <nav>
      ...
      <Link href={`/category?catgoryname=${param1}&page=${currentPage - 1}`} passHref>
        ...
        {pages.map(page => (
          <li key={page} style={{ margin: '0 5px' }}>
              <Link style={{ textDecoration: currentPage === page ? 'underline' : 'none' }} href={`/category?catgoryname=${param1}&page=${page}`}>
                {page}
              </Link>
